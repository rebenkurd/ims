...
export const useCartStore = defineStore('cart', {
  ...
  actions: {
    ...
    addItemDirect(item) {
      this.items.push({
        ...item,
        unitCost: parseFloat(item.unit_cost || item.purchasePrice || 0),
        totalAmount: parseFloat(item.total_amount || (item.quantity * item.purchasePrice) || 0)
      });
    },
    
    syncWithPurchase(purchase) {
      this.clearItems();
      ...
      this.discountAll = purchase.discount_all || 0;
      this.discountType = purchase.discount_type || 'fixed';
    ...
    updateItemTotals(item) {
      ...
      // Calculate values
      const basePrice = purchasePrice * quantity;
      item.totalAmount = basePrice - discount;
      item.unitCost = quantity > 0 ? item.totalAmount / quantity : 0;
